...
func TestToggleMarkAddsAndRemoves(t *testing.T) {
	...
	m.local.cursor = 1 // a.txt

	newModel, _ := m.toggleMark()
	got := newModel.(model)
	if !got.local.isSelected(1) {
		t.Fatal("expected index 1 to be marked after toggling")
	}

	newModel, _ = got.toggleMark()
	got = newModel.(model)
	if got.local.isSelected(1) {
		t.Fatal("expected index 1 to be unmarked after toggling again")
	}
}
...
func TestEnterVisualThenExtendRangeThenYank(t *testing.T) {
	...
	// extend down twice: cursor moves to c.txt (index 3), range = [1,2,3]
...
func TestVisualEscRestoresPriorSelection(t *testing.T) {
	...
	m.local.cursor = 2 // enter visual anchored at b.txt
	newModel, _ = m.enterVisual()
	got := newModel.(model)

	newModel, _ = got.updateVisual(keyMsg("down")) // extend to c.txt: marks 2,3 added
	got = newModel.(model)
	if !got.local.isSelected(3) {
		t.Fatal("expected index 3 to be selected mid-range")
	}
	...
	if !got.local.isSelected(1) || got.local.isSelected(2) || got.local.isSelected(3) {
		t.Fatalf("selected after esc = %v, want only index 1 (the pre-existing mark)", got.local.selected)
	}
}
